...
export function LogIn() {
  ...
  async function handleSignInWithEmailandPassword(e: FormEvent) {
    ...
    await signInWithEmailAndPassword(email, password).then(handleLoggedRedirect);
  };
  ...
  if (error) {
    return (
      <div className="flex flex-col gap-4 items-center justify-center">
        ...
        <Button onClick={() => window.location.reload(true)}>Retornar</Button>
      </div>
    );
  }
  if (loading) {
  ...
  return (
    <div className='w-full h-screen text-gray-100 flex flex-col items-center justify-center bg-gray-900'>
      ...
      <form onSubmit={handleSignInWithEmailandPassword} className='flex flex-col gap-4 items-stretch mt-8 w-full max-w-sm'>
        <label htmlFor='email' className='flex flex-col gap-3'>
          ...
          <TextInput.Root>
            ...
            <TextInput.Input autoComplete="email" onChange={(e) => setEmail(e.target.value)} type='email' id='email' placeholder='Digite seu e-mail' />
          </TextInput.Root>
        </label>
        <label htmlFor='password' className='flex flex-col gap-3'>
          ...
          <TextInput.Root>
            ...
            <TextInput.Input autoComplete="password" onChange={(e) => setPassword(e.target.value)} type='password' id='password' placeholder='Digite sua senha' />
          </TextInput.Root>
        </label>

        <label htmlFor='remember' className='flex items-center gap-2'>
          <Checkbox id='remember' checked={isRememberChecked} onCheckedChange={setIsRememberChecked} />
      ...
      <footer className='mt-4 flex flex-col items-center gap-2 mb-8'>
        ...
        <Text asChild size='xsm'>
          <Link to="/register" className='text-gray-400 underline transitions-colors hover:text-gray-200'>Não possui conta? Crie uma agora!</Link>
        </Text>
      </footer>
    </div>
  )
